...
class Scan_1(QThread):   # 类Scan_1继承自 QThread
    ...
    def mssql_scan(self,f_name,db_name,logging,start_off,end_off,scan_size): # 源文件名，扫描的起始字节，size，输出的sqlite数据库名
        ...
        sum = 0 ; page_1 = 0 ; page_15 = 0 ; page_2 = 0 ;page_3 = 0 ;page_10 = 0 ;step = 0
        ...
        for i in range(0,loop_1+1):  # 扫描文件的所有buffer块
                ...
                for ii in range(0,loop_2):
                    ...
                    if chk1 == 1 :
                        ...
                        if chk == 1 :   # 统计信息
                            ...
                            sum += 1
                            step = 15
                            cursor.execute("insert into mssql_page(offset,page_type,obj_id,ind_id,page_no,file_no) values(?,?,?,?,?,?)",(offset,data2[1],data2[12],data2[5],data2[15],data2[16]))
            ...
            if i%(loop_1//1000+1) == 0 or i==loop_1:  # 输出扫描进度
                ...
                print("Buffer:%d/%d; Percent:%5.1f%%; Find:%d, %dM, I/O:%4.1fM/s\r"%(i,loop_1,progress,sum,sum*8//1024,i*8/(now-begin+1)),end="")
                self.PUP.emit(progress)                  # 传递参数
                self.LUP.emit(" Buf:%d/%d,Percent:%4.1f%%, Find:%d=%dM, I/O:%4.1fM/s,Time:%dMin"%(i,loop_1,progress,sum,sum*page_size/1024/1024,speed,((loop_1+1-i)*8/((speed+0.01)*60)+1)))   # 传递参数


        print ("\n总页数:%d; 数据页:%d,索引页:%d,LOB页:%d,IAM页:%d,文件头:%d "%(sum,page_1,page_2,page_3,page_10,page_15))   # 总页数,空页数,数据页数
        logging.info("\n总页数:%d; 数据页:%d,索引页:%d,LOB页:%d,IAM页:%d,文件头:%d "%(sum,page_1,page_2,page_3,page_10,page_15))
        conn.commit();cursor.close();conn.close();f.close()
        end = time.time()
        ...
        logging.info("Datetime: " + time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(end))+"\t 扫描完成,用时: %d:"%((end-begin)//3600) + time.strftime('%M:%S',time.localtime(end-begin)))
        logging.info("File size:%6.2fG, 平均I/O:%4.1fM/s"%(f_size/1024/1024/1024,f_size/1024/1024/(end-begin+1)))
