...
def file_info_1(data,version):
    ...
    if version >= 706:  # sql2012/ sql2014/   # 此version是 创建版本
        f_no,g_no,f_size1 = s("<2HI",data[slot[0]+127:slot[0]+135])
    elif version >= 655 and version < 706 :   # sql2008 sql2008r2   127
        f_no,g_no,f_size1 = s("<2HI",data[slot[0]+126:slot[0]+134])  # 与版本有关=========
    elif version == 611 :  # sql2005
        f_no,g_no,f_size1 = s("<2HI",data[slot[0]+122:slot[0]+130])
    elif version == 539:  # sql2000
        f_no,g_no,f_size1 = s("<2HI",data[slot[0]+75:slot[0]+83])
    elif version < 539:  # sql 7
        f_no,g_no,f_size1 = s("<2HI",data[slot[0]+63:slot[0]+71])
    else:
        f_no,g_no,f_size1 = s("<2HI",data[slot[0]+126:slot[0]+134])
    f_size = f_size1*8/1024
    print("f_no:%d, g_no:%d, f_size:%d page,%4.1f M, file_type:%s "%(f_no,g_no,f_size1,f_size,file_type))
...
def file_info_all(f,pos,page_no_1,page_no_2):
    data1 = []; data2 = []; data3 = []
    file_type = '';f_name = '';f_path = '';
    f_no= 0; g_no = 0;f_size1 = 0; version1 = 661;version2 =661
    ...
    if page_no_1 == 0 :
        ...
        if pos >24 :  # 是否是bak文件
            f.seek(pos-24)
            data = f.read(24)
            data11 = s("<I",data[0:4])  # 1094996301
            data0 = data11[0]
        if data0 == 1094996301 :
            file_type = 'bak'       # 只能识别到非压缩的bak
            print("this is a bak file ===============offset: %d ====="%pos)
        else: print("this is a data file ===============offset: %d ====="%pos)
        f.seek(pos)
        if page_no_2 < 9 :
            data = f.read(8192)
    ...
    version1 = 661 ; db_name = ''
    if len(data2) :
        chk_type = s("B", data2[5:6])
        if chk_type[0]%4 == 1 : # 处理残缺页校验的页数据
            data2 = page_handle(data2)
        db_name,version1,version2 = file_info_2(data2) # boot页，获取数据库名称，数据库版本.
        print("version1:%d, db_name:%s "%(version1,db_name))

    if len(data1) :
        chk_type = s("B", data1[5:6])
        if chk_type[0]%4 == 1 : # 处理残缺页校验的页数据
            data1 = page_handle(data1)
        f_no,g_no,f_size1,file_type = file_info_1(data1,version2)    # 首页，获取文件的大小
    if len(data3) :
        chk_type = s("B", data3[5:6])
        if chk_type[0]%4 == 1 : # 处理残缺页校验的页数据
            data3 = page_handle(data3)
        f_name,f_path = file_info_3(data3)             # 08表，获取数据库文件路径,32页
    file_info1.file_name = os.path.basename(f.name)
    file_info1.file_type = file_type
    file_info1.db_name = db_name
    file_info1.f_name = f_name
    file_info1.f_path = f_path
...
def get_info(db_name,f_name):  # sqlite数据库名，源文件名
    ...
    cursor = conn.cursor()
    cursor.execute("select offset_1,page_no_1,page_no_2,file_no_1,page_sum from link_1 where page_no_1<10 and page_no_2>=0 order by page_sum desc") # 从link1中读记录
    ...
    for i in range(0,len1):
        ...
        if page_no_1 < 10 :
            a += 1
            print('\nID:%d ,'%a, end='')
            file_info1 = file_info_all(f,pos,page_no_1,page_no_2)
            file_infos.append(file_info1)
    conn.close()
    f.close()
    return file_infos


# f_name = r'E:\VMFS_1.img'
# db_name = r'E:\zy\code\aa_2.db'
# get_info(db_name,f_name)   # 测试
